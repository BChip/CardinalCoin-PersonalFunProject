...
// Copyright (c) 2013 CardinalCoin developers
...
int64 CTransaction::nMinTxFee = 1000000;  // HIGH TX FEES AT START AND LOWER OVER TIME
...
const string strMessageMagic = "CardinalCoin Signed Message:\n";
...
//Maybe do a random coin amount per block
static const int64 nStartSubsidy = 250 * COIN;
static const int64 nMinSubsidy = 50 * COIN;

//Look into this more, try and get random halving with high burst blocks
...
static const int64 nMaxAdjustDown = 10; // 20% adjustment down
static const int64 nMaxAdjustUp = 5; // 10% adjustment up
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CardinalCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CardinalCoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("CardinalCoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running CardinalCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CardinalCoinMiner terminated\n");
        throw;
    }
}
